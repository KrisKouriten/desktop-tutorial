...
def test_like_for_like():
    ...
    ros_df = pd.DataFrame([
        {"store_id": "S001", "sku": "SKU_OLD", "ros": 5.0, "total_sold": 40, "active_weeks": 8},
        {"store_id": "S002", "sku": "SKU_OLD", "ros": 3.0, "total_sold": 24, "active_weeks": 8},
    ])
...
def test_category_fallback():
    ...
    ros_df = pd.DataFrame([
        {"store_id": "S001", "sku": "SKU_A", "ros": 4.0, "total_sold": 32, "active_weeks": 8},
        {"store_id": "S001", "sku": "SKU_B", "ros": 6.0, "total_sold": 48, "active_weeks": 8},
    ])
    sku_master = pd.DataFrame([
    ...
    # Category avg = (4+6)/2 = 5.0
    assert new_row["ros"] == 5.0


def test_no_new_skus():
    """When there are no new SKUs, ROS DataFrame is unchanged."""
    ros_df = pd.DataFrame([
        {"store_id": "S001", "sku": "SKU001", "ros": 5.0, "total_sold": 40, "active_weeks": 8},
    ])
    sku_master = pd.DataFrame([
...
def test_new_sku_only_in_ranged_stores():
    ...
    # SKU_NEW only ranged in S001, not S002
    store_range = pd.DataFrame([
        {"store_id": "S001", "sku": "SKU_OLD"},
        {"store_id": "S002", "sku": "SKU_OLD"},
        {"store_id": "S001", "sku": "SKU_NEW"},
