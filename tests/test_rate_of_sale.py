...
def test_basic_ros():
    ...
    assert "demand_std" in result.columns


def test_weighted_ros():
    """Weighted ROS should give more weight to recent weeks."""
    sales = _make_sales([
        ("2026-04-05", "S001", "SKU001", 20),  # Most recent — highest weight
        ("2026-03-29", "S001", "SKU001", 10),
        ("2026-03-22", "S001", "SKU001", 5),
        ("2026-03-15", "S001", "SKU001", 5),   # Oldest — lowest weight
    ])
    sales["week_ending"] = pd.to_datetime(sales["week_ending"])
    store_range = _make_range([("S001", "SKU001")])

    # Simple average = 10.0
    simple = calculate_ros(sales, store_range, ros_weeks=4, ros_floor=0.0, use_weighted=False)
    assert simple.iloc[0]["ros"] == 10.0

    # Weighted should be > 10 because recent weeks (higher sales) get more weight
    weighted = calculate_ros(sales, store_range, ros_weeks=4, ros_floor=0.0, use_weighted=True)
    assert weighted.iloc[0]["ros"] > 10.0
...
def test_demand_std_calculated():
    """Demand standard deviation should be calculated for safety stock."""
    sales = _make_sales([
        ("2026-04-05", "S001", "SKU001", 20),
        ("2026-03-29", "S001", "SKU001", 10),
        ("2026-03-22", "S001", "SKU001", 5),
        ("2026-03-15", "S001", "SKU001", 5),
    ])
    sales["week_ending"] = pd.to_datetime(sales["week_ending"])
    store_range = _make_range([("S001", "SKU001")])

    result = calculate_ros(sales, store_range, ros_weeks=4, ros_floor=0.0)
    assert result.iloc[0]["demand_std"] > 0  # Should have variability
