...
For new SKUs without sales history, borrow ROS and demand variability
from the SKU they replace. Falls back to category average if no replacement is mapped.
"""
import pandas as pd
...
def apply_newness(ros_df, sku_master_df, store_range_df):
    ...
    1. If replaces_sku is set, use that SKU's ROS and demand_std per store
    2. If replaces_sku also has no ROS (chained replacement), walk up to 2 levels
    3. Fall back to category average ROS per store

    Args:
        ros_df: Current ROS with columns [store_id, sku, ros, demand_std, total_sold, active_weeks]
    ...
    # Compute category average ROS and demand_std per store (excluding new SKUs)
    existing_ros = ros_df[~ros_df["sku"].isin(new_skus["sku"])].copy()
    existing_ros["category"] = existing_ros["sku"].map(sku_to_category)

    has_demand_std = "demand_std" in existing_ros.columns

    cat_avg = (
        existing_ros[existing_ros["ros"] > 0]
        .groupby(["store_id", "category"], as_index=False)
        .agg(
            cat_avg_ros=("ros", "mean"),
            cat_avg_std=("demand_std", "mean") if has_demand_std else ("ros", lambda x: 0.0),
        )
    )
    ...
    for _, row in new_range.iterrows():
        ...
        for _ in range(2):
            ...
            if not match.empty and match.iloc[0]["ros"] > 0:
                ...
                if has_demand_std:
                    std_value = match.iloc[0]["demand_std"]
                break
        ...
        if ros_value == 0.0:
            ...
            cat_match = cat_avg[
                (cat_avg["store_id"] == store_id) & (cat_avg["category"] == category)
            ]
            if not cat_match.empty:
                ros_value = round(cat_match.iloc[0]["cat_avg_ros"], 2)
                std_value = round(cat_match.iloc[0]["cat_avg_std"], 2)

        new_row = {
            "store_id": store_id,
            "sku": sku,
            "ros": ros_value,
            "demand_std": std_value,
            "total_sold": 0,
            "active_weeks": 0,
        }
